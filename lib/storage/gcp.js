...
module.exports = function() {
  ...
  var self = {
    ...
    //   }
      client = Storage({
        projectId: options.GCLOUD_PROJECT
      });
    ...
    copyIn: function(localPath, path, options, callback) {
      ...
      return client.upload.then( params => callback(params) );
    },
    ...
    remove: function(path, callback) {
      return client.delete(path).then( params => callback(params));
    },
    ...
    getUrl: function(path) {
      return (https ? 'https://' : 'http://') + 'storage.googleapis.com/' + bucket + '/';
    }
  };
  return self;
};
